...
(function ($) {
  ...
  var createSettingWindow = function () {
	...
	div.id="addClassCourseTimeInfo-dialog";
	div.tabindex="-1";
	div.role="dialog";
	div.dataset.backdrop="static";
	div.dataset.keyboard="false";
    return $(div);
  }
  ...
  function CalendarPrice(opts) {
    ...
    // 日历单日设置窗口
    if($("#addClassCourseTimeInfo-dialog").length>0){
    	this.settingWindow = $("#addClassCourseTimeInfo-dialog");
    }else{
  ...
  fn.createCalendar = function () {
    ...
    if (!this.opts.hideFooterButton) {
      ...
      html += '        <button class="btn btn-reset" disabled="true">重置</button>';
      html += '        <button class="btn btn-confirm" disabled="true">确定</button>';
      html += '        <button class="btn btn-cancel" disabled="true">取消</button>';
      html += '    </div>';
    }
    html += '</div>';
      ...
      for (var k = 1; k <= 7; k++){
        ...
        if (d > 0 && d <= thisMonthDays){
          ...
          if (tdId >= startDay && tdId <= endDay) {
            html += '<td class="valid-hook" data-week="' + (k - 1) + '" data-id="' + tdId + '" data-course="' + this.opts.course + '" data-teacher="' + this.opts.teacher + '"  data-classCourseId="' + this.opts.classCourseId + '"><b>' + d + '</b><div class="data-hook"></div></td>';
          } else {
...
debugger;
	...
	html += '		<div class="modal-content cddsw-container">'
	html += '			<div class="modal-header">'
	html += '				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
	html += '				<h4 class="modal-title cddsw-title" id="myModalLabel">0000-00-00</h4>'
	html += '			</div>'
    html += '   		<div class="modal-body cddsw-batch-settings">';
    ...
    html += '       		<div class="bs-content"><div class="row">';
    html += '           		<lable class="bs-lable col-sm-2">日期范围</lable>';
    html += '           		<div class=" bs-options-wrapper col-sm-10" style="display:flex;justify-content:space-around">';
    html += '						<input class="itext" name="startDay" id="startDay" readonly style="width:120px" type="text" class="form-control Wdate  {required:true}" onclick="var date = myMinDate();WdatePicker({dateFmt: \'yyyy-MM-dd\',maxDate:\'#F{$dp.$D(\\\'endDay\\\')||$dp.$D(\\\'classCloseTime\\\')}\',minDate:date});" />';
    //html += '               		 <input class="itext" name="startDay" placeholder="格式:年-月-日,例如:1970-01-01" type="text" style="width:120px">';
    html += '               		<span class="white-space">-</span>';
    html += '							<input class="itext" name="endDay" id="endDay" readonly type="text" style="width:120px" class="form-control Wdate  {required:true}" onclick="myMinEndDate();WdatePicker({dateFmt: \'yyyy-MM-dd\',minDate:\'#F{$dp.$D(\\\'startDay\\\')||$dp.$D(\\\'tempBindTime\\\')}\',maxDate:\'#F{$dp.$D(\\\'classCloseTime\\\')}\'});" />';
    //html += '               		<input class="itext" name="endDay" placeholder="格式:年-月-日,例如:1970-01-01" type="text" style="width:120px">';
    html += '               		<label class="drw-enable"><input name="enableDateRange" type="checkbox">&nbsp;启用</label>';
    html += '           		</div>';
    html += '      			</div></div>';
    html += '       		<div class="bs-content bs-date-flexset"><div class="row">';
    html += '           		<lable class="bs-lable col-sm-2">设置星期</lable>';
    html += '           		<div class="bs-options-wrapper col-sm-10" style="display:flex;justify-content:space-around">';
    ...
    html += '       	<button class="btn btn-primary saveSetting">启用本设置</button>';
    html += '       	<button type="button" class="btn btn-cancel" data-dismiss="modal">返回</button>'
    html += '   	</div>';
    html += '	</div>';
  ...
  fn._createDaySetupInputGroup = function () {
    ...
    for (var i = 0; i < config.length; i++) {
	  ...
	  if(val.status=='hidden'){
		  html += '<input name="'+ val.key +'" type="hidden">';
		  continue;
	  }
	  if(even%2==1){
		  html += '<div class="form-group"><div class="row">';
		  html += '   <label class="col-sm-3 control-label"><span style="color:red; font-weight:bold;">*</span>'+ val.name +'</label><div class="col-sm-3">';
	  }else{
		  html += '   <label class="col-sm-2 control-label"><span style="color:red; font-weight:bold;">*</span>'+ val.name +'</label><div class="col-sm-3">';
	  }
      
	  
	  if(val.key.match("Time")){
		  html += '   <input class="form-control" name="'+ val.key +'" placeholder="格式:时:分:秒,例如:00:00:00" type="text">';
	  }else if(val.status=='disabled'){
			html += '   <input class="form-control" name="'+ val.key +'" type="text" disabled>';
		}else{
			html += '   <input class="form-control" name="'+ val.key +'" type="text">';
		}
	  if(even%2==0){
		  html += '</div></div>';
	  }
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.on('click', '.valid-hook', function () {
	  ...
	  var couTemp = $(this).data('course');
	  var teaTemp = $(this).data('teacher');
	  var claCouTemp = $(this).data('classcourseid');
      // 当前日的数据
	  ...
	  me.settingWindow.find('[name="courseName"]').val(couTemp);
	  me.settingWindow.find('[name="lecturerName"]').val(teaTemp);
	  me.settingWindow.find('[name="classCourseId"]').val(claCouTemp);

      me.settingWindow.modal("show");
    });

    // ** 单日详情设置窗口内按钮点击事件 *******************************************
    // 关闭设置框
    this.settingWindow.on('click', '.cddsw-close, .btn-cancel', function () {
      me.settingWindow.modal("hide");
      // $(this).closest('.capricorncd-date-detailed-settings').hide();
    });

    // 保存设置
    this.settingWindow.on('click', '.saveSetting', function () {
    var Timers = $("#addClassCourseTimeInfo-dialog").find('[name="startTime"], [name="endTime"]');
    ...
    $("#addClassCourseTimeInfo-dialog").find('.btn-reset').prop("disabled",false);
    $("#addClassCourseTimeInfo-dialog").find('.btn-confirm').prop("disabled",false);
    $("#addClassCourseTimeInfo-dialog").find('.btn-cancel').prop("disabled",false);
