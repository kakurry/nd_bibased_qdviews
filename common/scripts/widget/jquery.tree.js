...
(function($) {
	...
	//tree构造函数
		...
		_init: function() {
			...
			setting.treeObjId = this.element.attr("id");
			setting.treeObj = this.element;
			setting.root.tId = -1;
			...
			if (setting.nodes) {
				setting.root[setting.nodesCol] = setting.nodes;
			}
			if (setting.isSimpleData) {
				setting.root[setting.nodesCol] = transformToTreeFormat(setting, setting.root[setting.nodesCol]);
			}
			settings[setting.treeObjId] = setting;

			setting.treeObj.empty();

			bindTreeNodes(setting, this.element);
		...
		transformToTreeNodes : function(simpleTreeNodes) {
			return transformToTreeFormat(this.setting, simpleTreeNodes);
		},		
		...
		getSetting : function() {
			...
			var tmp_checkType = treeSetting.checkType;
			treeSetting.checkType = undefined;
			var tmp_callback = treeSetting.callback;
			treeSetting.callback = undefined;
			var tmp_root = treeSetting.root;
			treeSetting.root = undefined;
				
			$.extend(setting, treeSetting);
			
			treeSetting.checkType = tmp_checkType;				
			treeSetting.callback = tmp_callback;				
			treeSetting.root = tmp_root;				
				//不能获取root信息
		...
		updateSetting : function(treeSetting) {
			if (!treeSetting) return;
			var setting = this.setting;
	...
	function initTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0; i < treeNodes.length; i++) {
			...
			node.level = level;
			node.tId = setting.treeObjId + "_" + (++treeId);
	...
	function showTree(setting, treeNode) {
		...
		function(eventMouseDown) {
			...
			$(doc).mousemove(function(event) {
				...
				if (setting.dragStatus == 1 && tmpArrow.attr("id") != event.target.id) {
					...
					if (event.target.id == targetSetting.treeObjId || $(event.target).parents("#" + targetSetting.treeObjId).length>0) {
						//只有移动到Tree容器的边缘才算移到 根（排除根节点在本棵树内的移动）
			...
			//该 Bug 导致如果 Tree Div CSS 中存在 overflow 设置，则拖拽节点移出 zTree 时，无法得到正确的event.target
...
//		}
	...
	function addTreeNodes(setting, parentNode, newNodes, isSilent) {
		...
		if (setting.isSimpleData) {
			newNodes = transformToTreeFormat(setting, newNodes);
		}
		if (parentNode) {
			//目标节点必须在当前树内
	...
	//简要数据转换为标准JSON数组
	function transformToTreeFormat(setting, simpleTreeNodes) {
