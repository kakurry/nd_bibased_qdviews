...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		str+= '<input name="perms" type="checkbox" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')" value="'+node.title+'"/>';
		//alert(str);
	}
	if (node.url) {
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc) && node.url.indexOf('http:')==-1)	str += ' onclick="javascript: ' + this.obj + 'SelectMenu('+'\''+node.url+'\''+ ');"';
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc) && node.url.indexOf('http:')>-1)	str += ' onclick="javascript: window.open('+'\''+node.url.substring(node.url.indexOf('http:'))+'\''+ ') "';
...
//	var params ={"pid":cn.pid,"name":name};
//	$.ajax( {
//		type:"get",
