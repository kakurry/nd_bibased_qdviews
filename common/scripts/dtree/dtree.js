...
function dTree(objName, iconPath) {
	this.config = {
		...
		inOrder : false,
		check : false
	// added by wallimn 2009-02-05, to control whether checkbox is showed
	};
	// modified by wallimn 2009-02-05, in order to make client can modify path
	// of icons;
	// set the default value, be compatible with dtree
	this.icon = {
		root : iconPath + '/common/scripts/dtree/img/base.gif',
		folder : iconPath + '/common/scripts/dtree/img/folder.gif',
		folderOpen : iconPath + '/common/scripts/dtree/img/folderopen.gif',
		node : iconPath + '/common/scripts/dtree/img/page.gif',
		empty : iconPath + '/common/scripts/dtree/img/empty.gif',
		line : iconPath + '/common/scripts/dtree/img/line.gif',
		join : iconPath + '/common/scripts/dtree/img/join.gif',
		joinBottom : iconPath + '/common/scripts/dtree/img/joinbottom.gif',
		plus : iconPath + '/common/scripts/dtree/img/plus.gif',
		plusBottom : iconPath + '/common/scripts/dtree/img/plusbottom.gif',
		minus : iconPath + '/common/scripts/dtree/img/minus.gif',
		minusBottom : iconPath + '/common/scripts/dtree/img/minusbottom.gif',
		nlPlus : iconPath + '/common/scripts/dtree/img/nolines_plus.gif',
		nlMinus : iconPath + '/common/scripts/dtree/img/nolines_minus.gif'
	};
	// added by wallimn, to cache checkbox object and improve speed of set
	// checked status
 ...
 * 获取选中节点中子节点，父节点不需要
 */
dTree.prototype.getCheckedChildNodes = function() {
	var res = new Array();
	...
	for (var k = 0; k < res.length; k++) {
		for (var n = 0; n < this.aNodes.length; n++) {
			cko = this.co(this.aNodes[n].id);
			if (cko && cko.checked == true && this.aNodes[n].pid == res[k]) {
				breakFlag = true;
				break;
			} else {
				breakFlag = false;
		...
		resValue[resValue.length] = res[k];
	}
	return resValue;
}
...
dTree.prototype.getCheckedNodes=function(){
	...
	for (var n = 0; n < this.aNodes.length; n++) {
		...
		// cache the object to improve speed when you have very,very many nodes
		// and call this function many times in one page
...
dTree.prototype.getCheckedNodesAllId = function() {
	var res = new Array();
	var cko;// checkobject
	for (var n = 0; n < this.aNodes.length; n++) {
		// alert("c"+this.obj+n+(document.getElementById("c"+this.obj+n).checked));
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.check == true) {
		str += '<input type="checkbox" class="cx" id="c' + this.obj + nodeId
	...
	if (node.url) {
		...
		if (this.config.useSelection
				...
				&& node.url.indexOf('http:') == -1)
			str += ' onclick="javascript: ' + this.obj + 'SelectMenu(' + '\''
					+ node.url + '\'' + ');"';
		// if (this.config.useSelection && ((node._hc &&
		// this.config.folderLinks) || !node._hc) &&
		// node.url.indexOf('http:')>-1) str += ' onclick="javascript:
		// window.open('+'\''+node.url.substring(node.url.indexOf('http:'))+'\''+
		// ') "';
		if (this.config.useSelection
				&& ((node._hc && this.config.folderLinks) || !node._hc)
				&& node.url.indexOf('http:') > -1)
			str += ' onclick="javascript: winOpen' + this.obj + 'SelectMenu('
					+ '\'' + node.url + '\'' + ');"';
...
dTree.prototype.o = function(id) {
	var cn = this.aNodes[id];
	var path = this.icon.root.substring(0, this.icon.root.indexOf("common/"));
	var urlStr = path + "/operater/save/folder";
	var name = $.base64Encode(cn.name);
	var params = {
		"pid" : cn.pid,
		"name" : name
	};
	$.ajax({
		type : "get",
		url : urlStr + ".json" + new Date().getTime(),
		data : params,
		dataType : "json",
